...
.EXAMPLE
.\Collect-Floor6DeploymentEvidence_Commented.ps1 -OutputRoot C:\IR -DmsNamePattern "iManage|NetDocuments|DMS" -DryRun

.EXAMPLE
.\Collect-Floor6DeploymentEvidence_Commented.ps1 -OutputRoot C:\IR -LookbackDays 5 -IncludeSecurityLog
...
# These parameters control output location, collection scope, deployment matching, and read-only dry run behavior.
[CmdletBinding()]
param(
    [Parameter()]
...
# Section: Script safety defaults.
# Strict mode and stop-on-error make failures explicit and easier to audit in incident response.
Set-StrictMode -Version Latest
$ErrorActionPreference = 'Stop'

# Section: Script-scoped state.
# These variables track runtime timing, warnings, errors, artifact paths, and transcript location.
...
# Writes a timestamped status line to console for operator visibility.
...
# Records a non-fatal warning in both memory and console output.
...
# Records a fatal error message in both memory and error output stream.
...
# Executes a collection step safely.
# If the step fails, warning is recorded and a caller-provided default is returned.
...
# Creates a timestamped evidence folder unless DryRun is enabled.
# This preserves per-run chain of evidence and prevents overwrite.
...
# Exports any object to JSON as a named artifact.
# In DryRun mode, no file is written and intent is logged.
...
# Exports tabular evidence to CSV as a named artifact.
# In DryRun mode, no file is written and intent is logged.
...
# Collects installed application inventory from 64-bit HKLM, 32-bit HKLM, and HKCU uninstall keys.
# This provides broad software visibility, including user-scoped installs.
...
# Parses install dates in yyyyMMdd format from uninstall registry values.
# Returns null for empty/invalid formats to avoid false assumptions.
...
# Collects startup execution points from WMI startup commands and startup folders.
# Startup persistence is important for finding deployment-introduced logon/performance impact.
...
# Collects scheduled task inventory when cmdlet support is available.
# Tasks are common deployment and post-logon execution surfaces.
...
# Collects running process inventory with defensive property access.
# Some process properties throw access exceptions; these are handled without aborting collection.
...
# Captures point-in-time CPU, memory, and disk utilization.
# This provides objective evidence for user-reported performance degradation.
...
# Filters services by deployment-related patterns in name, display name, or binary path.
# This helps isolate potentially impacted components without changing service state.
...
# Enumerates available event log channels first.
# This avoids hard failures on endpoints missing certain operational logs.
...
# Reads targeted event logs with optional IDs, provider regex, start time, and max event caps.
# Returns normalized columns for CSV-friendly export.
...
# Collects user profile health evidence from WMI and ProfileList registry.
# Also checks known temporary profile event IDs and indicators.
...
# Collects desktop path status and shortcut inventory from current, user, and public desktop paths.
# Resolves .lnk targets when possible to detect path drift or broken shortcut destinations.
...
# Collects network, DNS, domain membership, secure-channel status, and auth-related events.
# This helps distinguish deployment impact from domain/network/authentication root causes.
...
# Collects folder redirection indicators and OneDrive context.
# Useful for desktop disappearance scenarios caused by path redirection changes.
...
# Scans key deployment-related paths for files modified after deployment window start.
# Matching is constrained by DMS regex pattern and capped to protect endpoint performance.
...
# Captures gpresult in text and HTML forms for policy processing evidence.
# gpresult output is metadata-linked for downstream review.
...
# Section: Main collection workflow.
# This block orchestrates all evidence steps, then writes structured artifacts and a summary.
try {
    Write-Status 'Starting deployment-impact evidence collection.'
    Write-Status "DryRun: $DryRun"

    # Create run-specific evidence root unless DryRun is requested.
    $script:EvidenceRoot = New-EvidenceFolder

    # Start transcript logging for operational traceability when writing artifacts.
    if (-not $DryRun) {
        $script:TranscriptPath = Join-Path -Path $script:EvidenceRoot -ChildPath 'Transcript.log'
        Start-Transcript -Path $script:TranscriptPath -Force | Out-Null
    }

    # Collect endpoint identity and OS baseline details.
    ...
    # Collect installed software and derive recently installed set by parsed install date.
    ...
    # Filter installed software to deployment-related matches using configurable regex pattern.
    $deploymentApps = $installedPrograms | Where-Object {
        $_.DisplayName -match $DmsNamePattern -or $_.Publisher -match $DmsNamePattern
    }

    # Collect startup, task, process, utilization, and deployment-linked service evidence.
    ...
    # Collect broad and login-related events in configured lookback window.
    ...
    # Optionally collect Security log authentication evidence when operator has rights.
    $securityLogons = @()
    ...
    # Collect policy, profile, desktop, network, redirection, and deployment-timestamp evidence.
    ...
    # Extract deployment-related errors from general event set.
    ...
    # Build collection summary with counts and any warnings/errors for quick triage.
    ...
    # Export all evidence artifacts in JSON/CSV formats for investigation and central aggregation.
    ...
    # Export an artifact index for quick navigation and automation-friendly ingestion.
    if (-not $DryRun) {
    ...
    # Fatal path: capture high-level failure and rethrow for caller visibility.
    Add-ErrorRecord "Unhandled failure: $($_.Exception.Message)"
    throw
}
finally {
    # Always attempt to close transcript cleanly when one was opened.
    ...
    # Emit runtime boundaries for response timelines and audit clarity.
    $ended = Get-Date
